...
class Simulation:
    def __init__(self, mass, surface_width, surface_height, x_init, v_init, s_coef_fric, k_coef_fric, block_display_dims, pid_init, wants_render=True, screen_dims=(5, 5)):
        ...
        if wants_render:
            ...
            self.iZone_text = 'IZone:' + str(pid_init["iZone"])
            self.kP_rect = pygame.Rect(100, 100, 140, 32)
            self.kI_rect = pygame.Rect(250, 100, 140, 32)
            self.kD_rect = pygame.Rect(400, 100, 140, 32)
            self.iZone_rect = pygame.Rect(550, 100, 140, 32)
            self.save_text = "Save"
            self.pid_save_rect = pygame.Rect(700, 100, 80, 32)
    ...
    def render(self, block_state, reference_x):
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.iZone_rect.collidepoint(event.pos):
                    self.active_textbox = "iZone_textbox"
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_BACKSPACE:
                    ...
                    if self.iZone_text[-1] != ":" and self.active_textbox == "iZone_textbox":
                        self.iZone_text = self.iZone_text[:-1]
                else:
                    ...
                    if len(self.iZone_text) <= 11 and self.active_textbox == "iZone_textbox":
                        self.iZone_text += event.unicode
        ...
        pygame.draw.rect(self.pyg_surface, ct.WHITE, self.iZone_rect)
        iZone_text_surface = self.font.render(self.iZone_text, True, ct.BLACK)
        ...
        self.pyg_surface.blit(iZone_text_surface, (self.iZone_rect.x+5, self.iZone_rect.y+5))
        ...
        self.iZone_rect.w = iZone_text_surface.get_width()+10
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.pid_save_rect.collidepoint(event.pos):
                    return {"kP":float(self.kP_text[3:]), "kI":float(self.kI_text[3:]), "kD":float(self.kD_text[3:]), "iZone":float(self.iZone_text[6:])}
        return None
