...
class Controller_PID_Point2Point():
    def __init__(
        ...
        self.TILT_LIMITS = [
            (params['Tilt_limits'][0] / 180.0) * 3.14,
    ...
    def update(self):
        ...
        [x, y, z, x_dot, y_dot, z_dot, theta, phi, gamma, theta_dot,
            phi_dot, gamma_dot] = self.get_state(self.quad_identifier)
        x_error = dest_x - x
        y_error = dest_y - y
        z_error = dest_z - z
        ...
        dest_x_dot = self.LINEAR_P[0] * (x_error) + \
            self.LINEAR_D[0] * (-x_dot) + self.xi_term
        dest_y_dot = self.LINEAR_P[1] * (y_error) + \
            self.LINEAR_D[1] * (-y_dot) + self.yi_term
        dest_z_dot = self.LINEAR_P[2] * (z_error) + \
            self.LINEAR_D[2] * (-z_dot) + self.zi_term
        ...
        gamma_dot_error = (self.YAW_RATE_SCALER *
                           self.wrap_angle(dest_gamma - gamma)) - gamma_dot
        ...
        x_val = self.ANGULAR_P[0] * (theta_error) + \
            self.ANGULAR_D[0] * (-theta_dot) + self.thetai_term
        y_val = self.ANGULAR_P[1] * (phi_error) + \
            self.ANGULAR_D[1] * (-phi_dot) + self.phii_term
    ...
    def start_thread(self, update_rate=0.005, time_scaling=1):
        self.thread_object = threading.Thread(
            target=self.thread_run, args=(
                update_rate, time_scaling))
