...
# Constants
# Any positive number(Smaller is faster). 1.0->Real Time, 0.0->Run as fast
# as possible
TIME_SCALING = 0.3
...
def Single_Point2Point():
    ...
    ctrl.start_thread(
        update_rate=CONTROLLER_DYNAMICS_UPDATE,
    ...
    QUADCOPTERS = {
        'q1': {
            'position': [
                1, 0, 4], 'orientation': [
                0, 0, 0], 'L': 0.3, 'r': 0.1, 'prop_size': [
                    10, 4.5], 'weight': 1.2}, 'q2': {
                        'position': [
                            -1, 0, 4], 'orientation': [
                                0, 0, 0], 'L': 0.15, 'r': 0.05, 'prop_size': [
                                    6, 4.5], 'weight': 0.7}}
    # Controller parameters
...
def parse_args():
    ...
    parser.add_argument(
        "--sim",
        help='single_p2p, multi_p2p or single_velocity',
        default='single_p2p')
    parser.add_argument(
        "--time_scale",
        type=float,
