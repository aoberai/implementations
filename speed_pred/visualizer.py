...
def preprocess(frame):
    ...
    Visualization
    '''
    cv2.imshow("Frame", cv2.resize(frame, tuple(
        [scale_factor := 7 * i for i in ct.IMG_SIZE[0:2]])))
    cv2.imshow(
        "Speed Predictor",
        cv2.resize(cv2.cvtColor(opflow_frame.astype('float32'), cv2.COLOR_GRAY2BGR), (ct.IMG_SIZE[0] * 3, ct.IMG_SIZE[1] * 3)))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(model_frames) > ct.TIMESTEPS:
            ...
            cv2.imshow(
                "Speed Predictor",
                cv2.resize(cv2.cvtColor(model_frames[-1].astype('float32') * 255, cv2.COLOR_GRAY2BGR), (ct.IMG_SIZE[0] * 3, ct.IMG_SIZE[1] * 3)))
            cv2.waitKey(1)
