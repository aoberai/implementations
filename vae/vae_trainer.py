...
def encoder(input_shape, output_latent_space_dims):
  ...
  log_var_vector = layers.Dense(output_latent_space_dims)(x)

  encoder = Model(inputs, (mean_vector, log_var_vector), name="Encoder")
...
def train_step(images):
    with tf.GradientTape() as encoder, tf.GradientTape() as decoder:
      ...
      loss += kl_divergence_loss(mean_vector, log_var_vector) # regularization: calculates kullback leibler divergence between normal distribution and distribution for each latent space variable
...
def train(dataset, epochs=10):
    ...
    mean_vector, log_var_vector = encoder_model.predict(x_test)

    # Sampling from gaussian latent space
    z_vector = mean_vector + tf.multiply(0.5 * tf.exp(log_var_vector), tf.random.normal(tf.shape(mean_vector), 0, 1, tf.float32))

    generated_images = decoder_model.predict(z_vector)

    print("Epoch 0 Validation Loss:", np.mean(reconstruction_loss(x_test, generated_images) + kl_divergence_loss(mean_vector, log_var_vector)))
    ...
    try:
        for epoch in range(epochs):
            ...
            validation_loss.append(np.mean(reconstruction_loss(x_test, generated_images) + kl_divergence_loss(mean_vector, log_var_vector)))
